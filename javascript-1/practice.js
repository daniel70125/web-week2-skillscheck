...
const doubled = myNumbers.map((elm) => {
	return elm * 2;
})
...
const filtered = myNumbers.filter((elm) => {
	return elm > 100;
})
...
const total = myNumbers.reduce((acc, elm) => {
	return acc + elm;
}, 0)
...
const forTheLoveOfGeorge = notGeorge.map((elm) => {
	return elm = "George";
})
...
const enemies = people.filter((elm) => {
	return elm.friend === false;
})
...
const totallyAwesome = people.reduce((acc, elm) => {
	return acc + elm.awesomeLevel;
}, 0)
